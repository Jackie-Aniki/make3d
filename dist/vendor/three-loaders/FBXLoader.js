...
const fflate = __importStar(require('../libs/fflate.module.js'));
const NURBSCurve_js_1 = require('../curves/NURBSCurve.js');
/**
...
let connections;
let sceneGraph;
class FBXLoader extends three_1.Loader {
  constructor(manager) {
    super(manager);
  }
  load(url, onLoad, onProgress, onError) {
    const scope = this;
    const path =
      scope.path === '' ? three_1.LoaderUtils.extractUrlBase(url) : scope.path;
...
class FBXTreeParser {
  ...
  parseTexture(textureNode, images) {
    ...
    texture.wrapS =
      valueU === 0 ? three_1.RepeatWrapping : three_1.ClampToEdgeWrapping;
    texture.wrapT =
      valueV === 0 ? three_1.RepeatWrapping : three_1.ClampToEdgeWrapping;
  ...
  loadTexture(textureNode, images) {
    ...
    const loader = nonNativeExtensions.has(extension)
      ? this.manager.getHandler(`.${extension}`)
      : this.textureLoader;
    if (!loader) {
      console.warn(
  ...
  parseMaterial(materialNode, textureMap) {
    ...
    switch (type.toLowerCase()) {
      case 'phong':
        material = new three_1.MeshPhongMaterial();
        break;
      case 'lambert':
  ...
  parseParameters(materialNode, textureMap, ID) {
    ...
    if (materialNode.Diffuse) {
      parameters.color = three_1.ColorManagement.toWorkingColorSpace(
        new three_1.Color().fromArray(materialNode.Diffuse.value),
        three_1.SRGBColorSpace
      );
    } else if (
      ...
      parameters.color = three_1.ColorManagement.toWorkingColorSpace(
        new three_1.Color().fromArray(materialNode.DiffuseColor.value),
        three_1.SRGBColorSpace
      );
    }
    ...
    if (materialNode.Emissive) {
      parameters.emissive = three_1.ColorManagement.toWorkingColorSpace(
        new three_1.Color().fromArray(materialNode.Emissive.value),
        three_1.SRGBColorSpace
      );
    } else if (
      ...
      parameters.emissive = three_1.ColorManagement.toWorkingColorSpace(
        new three_1.Color().fromArray(materialNode.EmissiveColor.value),
        three_1.SRGBColorSpace
      );
    }
    ...
    parameters.opacity =
      ...
      (materialNode.TransparencyFactor
        ? parseFloat(materialNode.TransparencyFactor.value)
        : 0);
    if (parameters.opacity === 1 || parameters.opacity === 0) {
      parameters.opacity = materialNode.Opacity
        ? parseFloat(materialNode.Opacity.value)
        : null;
    ...
    if (materialNode.Specular) {
      parameters.specular = three_1.ColorManagement.toWorkingColorSpace(
        new three_1.Color().fromArray(materialNode.Specular.value),
        three_1.SRGBColorSpace
      );
    } else if (
      ...
      parameters.specular = three_1.ColorManagement.toWorkingColorSpace(
        new three_1.Color().fromArray(materialNode.SpecularColor.value),
        three_1.SRGBColorSpace
      );
    }
    const scope = this;
      ...
      switch (type) {
        ...
        case 'Maya|TEX_color_map':
          ...
          if (parameters.map !== undefined) {
            parameters.map.colorSpace = three_1.SRGBColorSpace;
          }
          break;
        ...
        case 'EmissiveColor':
          ...
          if (parameters.emissiveMap !== undefined) {
            parameters.emissiveMap.colorSpace = three_1.SRGBColorSpace;
          }
          break;
        case 'NormalMap':
        ...
        case 'ReflectionColor':
          ...
          if (parameters.envMap !== undefined) {
            parameters.envMap.mapping =
              three_1.EquirectangularReflectionMapping;
            parameters.envMap.colorSpace = three_1.SRGBColorSpace;
          }
          break;
        case 'SpecularColor':
          parameters.specularMap = scope.getTexture(textureMap, child.ID);
          if (parameters.specularMap !== undefined) {
            parameters.specularMap.colorSpace = three_1.SRGBColorSpace;
          }
          break;
  ...
  parseSkeleton(relationships, deformerNodes) {
    ...
    relationships.children.forEach(function (child) {
      ...
      const rawBone = {
        ...
        indices: [],
        weights: [],
        transformLink: new three_1.Matrix4().fromArray(boneNode.TransformLink.a)
    ...
    for (const nodeID in modelNodes) {
      ...
      if (!model) {
        switch (node.attrType) {
          ...
          case 'LimbNode':
          case 'Root':
            model = new three_1.Bone();
            break;
          case 'Null':
          default:
            model = new three_1.Group();
            break;
        }
        model.name = node.attrName
          ? three_1.PropertyBinding.sanitizeNodeName(node.attrName)
          : '';
      ...
      for (const ID in skeletons) {
        ...
        skeleton.rawBones.forEach(function (rawBone, i) {
          if (rawBone.ID === parent.ID) {
            const subBone = bone;
            bone = new three_1.Bone();
            bone.matrixWorld.copy(rawBone.transformLink);
            // set name and id here - otherwise in cases where "subBone" is created it will not have a name / id
            bone.name = name
              ? three_1.PropertyBinding.sanitizeNodeName(name)
              : '';
  ...
  createCamera(relationships) {
    ...
    if (cameraAttribute === undefined) {
      ...
      switch (type) {
        case 0: // Perspective
          model = new three_1.PerspectiveCamera(
            fov,
            aspect,
            nearClippingPlane,
            farClippingPlane
  ...
  createLight(relationships) {
    ...
    if (lightAttribute === undefined) {
      ...
      if (lightAttribute.Color !== undefined) {
        color = three_1.ColorManagement.toWorkingColorSpace(
          new three_1.Color().fromArray(lightAttribute.Color.value),
          three_1.SRGBColorSpace
        );
      }
      let intensity =
      ...
      switch (type) {
        case 0: // Point
          model = new three_1.PointLight(color, intensity, distance, decay);
          break;
        case 1: // Directional
          model = new three_1.DirectionalLight(color, intensity);
          break;
        case 2: // Spot
          let angle = Math.PI / 3;
          if (lightAttribute.InnerAngle !== undefined) {
            angle = three_1.MathUtils.degToRad(lightAttribute.InnerAngle.value);
          }
          let penumbra = 0;
          if (lightAttribute.OuterAngle !== undefined) {
            ...
            penumbra = three_1.MathUtils.degToRad(
              lightAttribute.OuterAngle.value
            );
            penumbra = Math.max(penumbra, 1);
          }
          model = new three_1.SpotLight(
            color,
            intensity,
            distance,
            angle,
        ...
        default:
          ...
          model = new three_1.PointLight(color, intensity);
          break;
      }
      if (
  ...
  createMesh(relationships, geometryMap, materialMap) {
    ...
    if (materials.length > 1) {
      ...
      material = new three_1.MeshPhongMaterial({
        name: three_1.Loader.DEFAULT_MATERIAL_NAME,
        color: 0xcccccc
      });
    ...
    if (geometry.FBX_Deformer) {
      model = new three_1.SkinnedMesh(geometry, material);
      model.normalizeSkinWeights();
    } else {
      model = new three_1.Mesh(geometry, material);
    }
    return model;
  }
        ...
        if (child.relationship === 'LookAtProperty') {
          ...
          if ('Lcl_Translation' in lookAtTarget) {
            ...
            if (model.target !== undefined) {
              ...
              model.lookAt(new three_1.Vector3().fromArray(pos));
            }
          }
        }
      });
        ...
        if (geometryMap.has(parent.ID)) {
          ...
          geoRelationships.parents.forEach(function (geoConnParent) {
            if (modelMap.has(geoConnParent.ID)) {
              ...
              model.bind(
                new three_1.Skeleton(skeleton.bones),
        ...
        if (
          ...
          if (Array.isArray(poseNodes)) {
            poseNodes.forEach(function (poseNode) {
              bindMatrices[poseNode.Node] = new three_1.Matrix4().fromArray(
                poseNode.Matrix.a
              );
            });
          } else {
            bindMatrices[poseNodes.Node] = new three_1.Matrix4().fromArray(
              poseNodes.Matrix.a
            );
          }
        }
  ...
  addGlobalSceneSettings() {
    if ('GlobalSettings' in fbxTree) {
      if ('AmbientColor' in fbxTree.GlobalSettings) {
        ...
        if (r !== 0 || g !== 0 || b !== 0) {
          const color = new three_1.Color().setRGB(
            r,
            g,
            b,
            three_1.SRGBColorSpace
          );
          sceneGraph.add(new three_1.AmbientLight(color, 1));
        }
      }
      if ('UnitScaleFactor' in fbxTree.GlobalSettings) {
        sceneGraph.userData.unitScaleFactor =
          fbxTree.GlobalSettings.UnitScaleFactor.value;
      }
    }
  }
}
...
class GeometryParser {
  ...
  genGeometry(geoNode, skeleton, morphTargets, preTransform) {
    const geo = new three_1.BufferGeometry();
    if (geoNode.attrName) geo.name = geoNode.attrName;
    const geoInfo = this.parseGeoNode(geoNode, skeleton);
    const buffers = this.genBuffers(geoInfo);
    const positionAttribute = new three_1.Float32BufferAttribute(
      buffers.vertex,
      3
    );
    ...
    if (buffers.colors.length > 0) {
      geo.setAttribute(
        'color',
        new three_1.Float32BufferAttribute(buffers.colors, 3)
      );
    }
    if (skeleton) {
      geo.setAttribute(
        'skinIndex',
        new three_1.Uint16BufferAttribute(buffers.weightsIndices, 4)
      );
      geo.setAttribute(
        'skinWeight',
        new three_1.Float32BufferAttribute(buffers.vertexWeights, 4)
      );
      // used later to bind the skeleton to the model
      geo.FBX_Deformer = skeleton;
    }
    if (buffers.normal.length > 0) {
      const normalMatrix = new three_1.Matrix3().getNormalMatrix(preTransform);
      const normalAttribute = new three_1.Float32BufferAttribute(
        buffers.normal,
        3
      );
    ...
    buffers.uvs.forEach(function (uvBuffer, i) {
      ...
      geo.setAttribute(
        name,
        new three_1.Float32BufferAttribute(buffers.uvs[i], 2)
      );
    });
  ...
  getNormalNewell(vertices) {
    const normal = new three_1.Vector3(0.0, 0.0, 0.0);
  ...
  getNormalTangentAndBitangent(vertices) {
    ...
    const up =
      Math.abs(normalVector.z) > 0.5
        ? new three_1.Vector3(0.0, 1.0, 0.0)
        : new three_1.Vector3(0.0, 0.0, 1.0);
  ...
  genFace(
    ...
    if (faceLength > 3) {
      ...
      triangles = three_1.ShapeUtils.triangulateShape(triangulationInput, []);
    } else {
  ...
  genMorphGeometry(parentGeo, parentGeoNode, morphGeoNode, preTransform, name) {
    const basePositions =
      parentGeoNode.Vertices !== undefined ? parentGeoNode.Vertices.a : [];
    const baseIndices =
  ...
  parseVertexColors(ColorNode) {
    ...
    for (let i = 0, c = new three_1.Color(); i < buffer.length; i += 4) {
      c.fromArray(buffer, i);
      three_1.ColorManagement.toWorkingColorSpace(c, three_1.SRGBColorSpace);
      c.toArray(buffer, i);
    }
    return {
      dataSize: 4,
  ...
  parseNurbsGeometry(geoNode) {
    ...
    for (let i = 0, l = pointsValues.length; i < l; i += 4) {
      controlPoints.push(new three_1.Vector4().fromArray(pointsValues, i));
    }
    ...
    const curve = new NURBSCurve_js_1.NURBSCurve(
      degree,
      knots,
      controlPoints,
      startKnot,
    ...
    return new three_1.BufferGeometry().setFromPoints(points);
  }
}
...
class AnimationParser {
                ...
                if (modelID !== undefined) {
                  ...
                  const node = {
                    modelName: rawModel.attrName
                      ? three_1.PropertyBinding.sanitizeNodeName(
                          rawModel.attrName
                        )
                      : '',
                    ID: rawModel.id,
                  ...
                  if (!node.transform) node.transform = new three_1.Matrix4();
  ...
  addClip(rawClip) {
    ...
    return new three_1.AnimationClip(rawClip.name, -1, tracks);
  }
  generateTracks(rawTracks) {
    const tracks = [];
    let initialPosition = new three_1.Vector3();
    let initialScale = new three_1.Vector3();
    if (rawTracks.transform)
      rawTracks.transform.decompose(
        initialPosition,
        new three_1.Quaternion(),
        initialScale
      );
  ...
  generateVectorTrack(modelName, curves, initialValue, type) {
    ...
    return new three_1.VectorKeyframeTrack(
      modelName + '.' + type,
      times,
      values
    );
  ...
  generateRotationTrack(
    ...
    if (preRotation !== undefined) {
      preRotation = preRotation.map(three_1.MathUtils.degToRad);
      preRotation.push(defaultEulerOrder);
      preRotation = new three_1.Euler().fromArray(preRotation);
      preRotation = new three_1.Quaternion().setFromEuler(preRotation);
    }
    if (postRotation !== undefined) {
      postRotation = postRotation.map(three_1.MathUtils.degToRad);
      postRotation.push(defaultEulerOrder);
      postRotation = new three_1.Euler().fromArray(postRotation);
      postRotation = new three_1.Quaternion()
    ...
    if (!values || !times)
      return new three_1.QuaternionKeyframeTrack(
    ...
    for (let i = 0; i < values.length; i += 3) {
      ...
      // Check unroll
      if (i > 2) {
        const prevQuat = new three_1.Quaternion().fromArray(
          quaternionValues,
          ((i - 3) / 3) * 4
        );
  ...
  generateMorphTrack(rawTracks) {
    ...
    return new three_1.NumberKeyframeTrack(
  ...
  interpolateRotations(curvex, curvey, curvez, eulerOrder) {
    ...
    values.push(three_1.MathUtils.degToRad(curvex.values[0]));
    values.push(three_1.MathUtils.degToRad(curvey.values[0]));
    values.push(three_1.MathUtils.degToRad(curvez.values[0]));
    for (let i = 1; i < curvex.values.length; i++) {
      ...
      const initialValueRad = initialValue.map(three_1.MathUtils.degToRad);
      const currentValue = [
        curvex.values[i],
        curvey.values[i],
        curvez.values[i]
      ...
      const currentValueRad = currentValue.map(three_1.MathUtils.degToRad);
      const valuesSpan = [
      ...
      if (
        ...
        const E1 = new three_1.Euler(...initialValueRad, eulerOrder);
        const E2 = new three_1.Euler(...currentValueRad, eulerOrder);
        const Q1 = new three_1.Quaternion().setFromEuler(E1);
        const Q2 = new three_1.Quaternion().setFromEuler(E2);
        // Check unroll
        if (Q1.dot(Q2)) {
        ...
        values.push(three_1.MathUtils.degToRad(curvex.values[i]));
        values.push(three_1.MathUtils.degToRad(curvey.values[i]));
        values.push(three_1.MathUtils.degToRad(curvez.values[i]));
      }
    }
    return [times, values];
  }
...
const tempEuler = new three_1.Euler();
const tempVec = new three_1.Vector3();
// generate transformation from FBX transform data
// ref: https://help.autodesk.com/view/FBX/2017/ENU/?guid=__files_GUID_10CDD63C_79C1_4F2D_BB28_AD2BE65A02ED_htm
// ref: http://docs.autodesk.com/FBX/2014/ENU/FBX-SDK-Documentation/index.html?url=cpp_ref/_transformations_2main_8cxx-example.html,topicNumber=cpp_ref__transformations_2main_8cxx_example_htmlfc10a1e1-b18d-4e72-9dc0-70d0f1959f5e
function generateTransform(transformData) {
  const lTranslationM = new three_1.Matrix4();
  const lPreRotationM = new three_1.Matrix4();
  const lRotationM = new three_1.Matrix4();
  const lPostRotationM = new three_1.Matrix4();
  const lScalingM = new three_1.Matrix4();
  const lScalingPivotM = new three_1.Matrix4();
  const lScalingOffsetM = new three_1.Matrix4();
  const lRotationOffsetM = new three_1.Matrix4();
  const lRotationPivotM = new three_1.Matrix4();
  const lParentGX = new three_1.Matrix4();
  const lParentLX = new three_1.Matrix4();
  const lGlobalT = new three_1.Matrix4();
  ...
  if (transformData.preRotation) {
    const array = transformData.preRotation.map(three_1.MathUtils.degToRad);
    array.push(defaultEulerOrder);
    lPreRotationM.makeRotationFromEuler(tempEuler.fromArray(array));
  }
  if (transformData.rotation) {
    const array = transformData.rotation.map(three_1.MathUtils.degToRad);
    array.push(transformData.eulerOrder || defaultEulerOrder);
    lRotationM.makeRotationFromEuler(tempEuler.fromArray(array));
  }
  if (transformData.postRotation) {
    const array = transformData.postRotation.map(three_1.MathUtils.degToRad);
    array.push(defaultEulerOrder);
  ...
  // Global Rotation
  const lParentGRM = new three_1.Matrix4();
  lParentGRM.extractRotation(lParentGX);
  // Global Shear*Scaling
  const lParentTM = new three_1.Matrix4();
  lParentTM.copyPosition(lParentGX);
  const lParentGRSM = lParentTM.clone().invert().multiply(lParentGX);
  const lParentGSM = lParentGRM.clone().invert().multiply(lParentGRSM);
  const lLSM = lScalingM;
  const lGlobalRS = new three_1.Matrix4();
  if (inheritType === 0) {
    lGlobalRS
      .copy(lParentGRM)
      .multiply(lLRM)
    ...
    const lParentLSM = new three_1.Matrix4().scale(
      new three_1.Vector3().setFromMatrixScale(lParentLX)
    );
  ...
  const lLocalTWithAllPivotAndOffsetInfo = new three_1.Matrix4().copyPosition(
    lTransform
  );
